...
export async function placeOrderDomain(order: OrderRequest) {
  ...
  const bookMap: Record<BookID, number> = {}
  for (const item of order.items) {
...
export default function placeOrderRoute(router: ZodRouter): void {
  router.register({
